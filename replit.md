...
- **Auth**: Replit OIDC with authorization code + PKCE + nonce via `openid-client` and `iron-session`, using issuer `https://replit.com/oidc` and dynamic client registration. Login/callback routes derive the callback origin from the active request host so preview, mobile, and deployed domains keep the session cookie on the same host; Replit preview/app hosts are accepted explicitly for browser compatibility. Mobile Get Started, mobile menu, protected-route redirects, and mobile client sign-in calls route through `/sign-in?returnTo=...`, where users explicitly open Replit sign-in in a separate tab/window, the opener is nulled before leaving same-origin, and the ONA tab stays alive while polling `/api/auth/user?optional=1` with bounded retries. Desktop Get Started/client sign-in preserves the prior direct `/api/login?returnTo=...` start from the user's click, and desktop arrivals at `/sign-in` without an error automatically continue to `/api/login`. Callback success and sign-in error pages signal the waiting tab via BroadcastChannel/localStorage so users see completion, retry, or continue actions without relying on `window.opener`. The callback saves the session, returns a same-origin "continue to ONA" handoff page with a manual Continue button, never attempts to close mobile auth tabs, and gives desktop non-popup callbacks a short visible continuation page before redirecting. Session cookie: `replit_auth_session` with SameSite=Lax.
...
- `src/app/[locale]/(auth)/dashboard/user-profile/[[...user-profile]]/SettingsActions.tsx` — Settings actions including API key creation, usage stats, rate-limit display, and copy-paste API examples
...
- Responsive optimization: landing sections use smaller mobile grids/spacing/type scales and full-width mobile CTAs; the `/app` header, banner, prompt composer, and suggestion chips are tuned to avoid crowding on mobile while preserving the denser desktop layout.
...
- `api_keys` — hashed per-user API keys created from Settings for programmatic API access. Raw keys are only shown once and must be sent as `Authorization: Bearer <key>`. Each key tracks total requests, last use, and has a default 60 requests/hour limit.
- `api_key_rate_limits` — per-key rolling rate-limit window state for programmatic API access.
